...
def new_books():
    if current_user.check_visibility(constants.SIDEBAR_DOWNLOAD):
        ...
        entries = ub.session.query(ub.Books) \
            .filter(ub.Books.visible == True) \
            .filter(ub.Books.timestamp >= thirty_days_ago) \
            .order_by(ub.Books.timestamp.desc()) \
            .all()
        return render_title_template("list.html", entries=entries,
                                     folder="web.books_list", charlist=None,
                                     title=_("New Books"),
                                     page="newbooks", data="new", order="new")
    else:
        abort(404)
